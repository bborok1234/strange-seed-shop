...
const assetSpecs = [
  {
    assetId: "facility_expedition_gate_v1",
    kind: "facility"
  },
  {
    assetId: "facility_expedition_return_crate_v1",
    kind: "facility"
  },
  {
    assetId: "fx_expedition_return_reward_strip_v1",
    kind: "fx_strip",
    frames: 8,
    frameWidth: 96,
    frameHeight: 96,
    fps: 14,
    binding: "facility_expedition_gate.action.claim_return_crate"
  }
];
...
for (const spec of assetSpecs) {
  ...
  completedAssets.push({
    ...
    status: spec.kind === "fx_strip" ? "normalized_runtime_strip_requires_review" : "manifest_candidate_requires_alpha_review",
    ...
    notes: spec.kind === "fx_strip"
      ? "Issue #494 normalized strict runtime strip from gpt-image-2 source candidate; review required before manifest/runtime binding."
      : "Issue #494 gpt-image-2 opaque candidate with edge-connected checkerboard alpha cleanup; review required before manifest/runtime binding.",
    cleanup,
...
status.notes = Array.from(new Set([
  ...(status.notes ?? []),
  "issue_0494_expedition_gate_raster_review: 3 gpt-image-2 candidates generated with SEED_ASSET_IMAGE_BACKGROUND=opaque after transparent background was rejected; facilities alpha-cleaned and FX normalized to strict 8x96x96 strip for review."
]));
status.batches = {
  ...(status.batches ?? {}),
  issue_0494_expedition_gate_raster_review: {
    total: assetSpecs.length,
    completed: assetSpecs.length,
    missing: [],
    blocked: false,
...
console.log(JSON.stringify({ ok: true, processed: completedAssets }, null, 2));
