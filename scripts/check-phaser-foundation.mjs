...
const OUT_DIR = "reports/visual/issue-0552-moon-grove-greenhouse-path-map";
...
async function runSmoke() {
  ...
  try {
    ...
    await clickUntilAction(page, [[304, 620], [318, 610], [284, 606], [284, 646], [230, 606]], "납품");
    ...
    await clickUntilAction(page, [[204, 520], [204, 500], [204, 546], [204, 590], [132, 344]], "초승달순 심기");
    await page.waitForTimeout(140);
    ...
    const moonFenceRouteUnlocked = await page.evaluate(() => ({
      ...
      moonFenceUnlockedRouteId: window.__seedGardenMoonFenceUnlockedRouteId ?? "",
      moonFenceUnlockedMarkerVisible: window.__seedGardenMoonFenceUnlockedMarkerVisible ?? false,
      moonFenceExpeditionState: window.__seedGardenMoonFenceExpeditionState ?? "",
      moonFenceRewardLeaves: window.__seedGardenMoonFenceRewardLeaves ?? 0,
      moonFenceReturnCrateVisible: window.__seedGardenMoonFenceReturnCrateVisible ?? false,
      moonFenceRewardClaimed: window.__seedGardenMoonFenceRewardClaimed ?? false,
    ...
    await page.getByRole("button", { name: "월정 문 원정 보내기" }).click();
    await page.waitForTimeout(180);
    const moonFenceExpeditionTraveling = await page.evaluate(() => ({
      objective: document.querySelector('[data-testid="phaser-objective"]')?.textContent ?? "",
      railText: document.querySelector('[data-testid="phaser-action-rail"]')?.textContent ?? "",
      selectedText: document.querySelector(".selected-entity")?.textContent ?? "",
      expeditionState: window.__seedGardenExpeditionState ?? "",
    ...
    await page.screenshot({ path: `${OUT_DIR}/phaser-check-moon-fence-expedition-traveling-393.png`, fullPage: false });
    await page.waitForTimeout(360);
    const moonFenceExpeditionReturned = await page.evaluate(() => ({
      objective: document.querySelector('[data-testid="phaser-objective"]')?.textContent ?? "",
      railText: document.querySelector('[data-testid="phaser-action-rail"]')?.textContent ?? "",
      selectedText: document.querySelector(".selected-entity")?.textContent ?? "",
      expeditionState: window.__seedGardenExpeditionState ?? "",
    ...
    await page.screenshot({ path: `${OUT_DIR}/phaser-check-moon-fence-expedition-returned-393.png`, fullPage: false });
    await page.getByRole("button", { name: "월정 문 귀환 상자 열기" }).click();
    await page.waitForTimeout(220);
    const moonFenceExpeditionClaimed = await page.evaluate(() => ({
      objective: document.querySelector('[data-testid="phaser-objective"]')?.textContent ?? "",
      railText: document.querySelector('[data-testid="phaser-action-rail"]')?.textContent ?? "",
      leaves: document.querySelector('[data-hud="leaves"]')?.textContent ?? "",
      selectedText: document.querySelector(".selected-entity")?.textContent ?? "",
      ...
      moonGroveSourceSeedPlanted: window.__seedGardenMoonGroveSourceSeedPlanted ?? false,
      moonGroveSourcePlotId: window.__seedGardenMoonGroveSourcePlotId ?? "",
      moonGroveSourceRenderedAssetKey: window.__seedGardenMoonGroveSourceRenderedAssetKey ?? "",
      moonGroveSourceFxKey: window.__seedGardenMoonGroveSourceFxKey ?? "",
      receipts: window.__seedGardenReceipts ?? []
    }));
    ...
    await clickUntilAction(page, [[204, 546], [132, 344], [262, 396]], "월정 숲 심기");
    await page.waitForTimeout(140);
    const moonGrovePlantAction = await page.evaluate(() => ({
      objective: document.querySelector('[data-testid="phaser-objective"]')?.textContent ?? "",
      railText: document.querySelector('[data-testid="phaser-action-rail"]')?.textContent ?? "",
      selectedText: document.querySelector(".selected-entity")?.textContent ?? "",
      moonGroveSourceAcquired: window.__seedGardenMoonGroveSourceAcquired ?? false,
    ...
    await page.screenshot({ path: `${OUT_DIR}/phaser-check-moon-grove-plant-action-393.png`, fullPage: false });
    await page.getByRole("button", { name: "월정 숲 심기" }).click();
    await page.waitForTimeout(180);
    const moonGrovePlanted = await page.evaluate(() => ({
      objective: document.querySelector('[data-testid="phaser-objective"]')?.textContent ?? "",
      railText: document.querySelector('[data-testid="phaser-action-rail"]')?.textContent ?? "",
      selectedText: document.querySelector(".selected-entity")?.textContent ?? "",
      moonGroveSourceAcquired: window.__seedGardenMoonGroveSourceAcquired ?? false,
      ...
      moonGroveSourceSeedPlanted: window.__seedGardenMoonGroveSourceSeedPlanted ?? false,
      moonGroveSourcePlotId: window.__seedGardenMoonGroveSourcePlotId ?? "",
      moonGroveSourceSeedHarvested: window.__seedGardenMoonGroveSourceSeedHarvested ?? false,
      moonGroveDiscoveryRevealed: window.__seedGardenMoonGroveDiscoveryRevealed ?? false,
      ...
      moonGroveClueMapOpened: window.__seedGardenMoonGroveClueMapOpened ?? false,
      moonGroveClueMapCurrentNodeId: window.__seedGardenMoonGroveClueMapCurrentNodeId ?? "",
      moonGroveClueMapNextNodeId: window.__seedGardenMoonGroveClueMapNextNodeId ?? "",
      moonGroveClueMapLockedNodeId: window.__seedGardenMoonGroveClueMapLockedNodeId ?? "",
      moonGroveSourceRenderedAssetKey: window.__seedGardenMoonGroveSourceRenderedAssetKey ?? "",
    ...
    await page.screenshot({ path: `${OUT_DIR}/phaser-check-moon-grove-planted-393.png`, fullPage: false });
    await page.getByRole("button", { name: "돌보기" }).click();
    await page.waitForTimeout(140);
    await page.getByRole("button", { name: "돌보기" }).click();
    await page.waitForTimeout(180);
    const moonGroveReady = await page.evaluate(() => ({
      objective: document.querySelector('[data-testid="phaser-objective"]')?.textContent ?? "",
      railText: document.querySelector('[data-testid="phaser-action-rail"]')?.textContent ?? "",
      selectedText: document.querySelector(".selected-entity")?.textContent ?? "",
      moonGroveSourceAcquired: window.__seedGardenMoonGroveSourceAcquired ?? false,
    ...
    await page.screenshot({ path: `${OUT_DIR}/phaser-check-moon-grove-ready-393.png`, fullPage: false });
    await page.getByRole("button", { name: "월정 숲 수확" }).click();
    await page.waitForTimeout(220);
    const moonGroveHarvested = await page.evaluate(() => ({
      objective: document.querySelector('[data-testid="phaser-objective"]')?.textContent ?? "",
      railText: document.querySelector('[data-testid="phaser-action-rail"]')?.textContent ?? "",
      leaves: document.querySelector('[data-hud="leaves"]')?.textContent ?? "",
      selectedText: document.querySelector(".selected-entity")?.textContent ?? "",
      ...
      moonGroveForestPathPreviewVisible: window.__seedGardenMoonGroveForestPathPreviewVisible ?? false,
      moonGroveForestPathPreviewId: window.__seedGardenMoonGroveForestPathPreviewId ?? "",
      moonGroveClueMapAvailable: window.__seedGardenMoonGroveClueMapAvailable ?? false,
      moonGroveClueMapOpened: window.__seedGardenMoonGroveClueMapOpened ?? false,
      moonGroveClueMapCurrentNodeId: window.__seedGardenMoonGroveClueMapCurrentNodeId ?? "",
      moonGroveClueMapNextNodeId: window.__seedGardenMoonGroveClueMapNextNodeId ?? "",
      moonGroveClueMapLockedNodeId: window.__seedGardenMoonGroveClueMapLockedNodeId ?? "",
    ...
    await page.getByRole("button", { name: "숲길 지도 펼치기" }).click();
    await page.waitForTimeout(220);
    const moonGroveClueMap = await page.evaluate(() => ({
      objective: document.querySelector('[data-testid="phaser-objective"]')?.textContent ?? "",
      railText: document.querySelector('[data-testid="phaser-action-rail"]')?.textContent ?? "",
      selectedText: document.querySelector(".selected-entity")?.textContent ?? "",
      actorIds: window.__seedGardenActorIds ?? [],
    ...
    await page.screenshot({ path: `${OUT_DIR}/phaser-check-moon-grove-clue-map-opened-393.png`, fullPage: false });
    await page.getByTestId("phaser-view-mode-toggle").click();
    await page.waitForTimeout(180);
    const moonFenceSourceOverview = await page.evaluate(() => ({
      viewMode: window.__seedGardenViewMode ?? "",
      hudCollapsed: window.__seedGardenHudCollapsed ?? false,
      objectiveDisplay: getComputedStyle(document.querySelector('[data-testid="phaser-objective"]')).display,
      actionRailDisplay: getComputedStyle(document.querySelector('[data-testid="phaser-action-rail"]')).display,
      bodyScrollHeight: document.body.scrollHeight,
      documentScrollHeight: document.documentElement.scrollHeight,
      innerHeight: window.innerHeight,
      moonFenceNextClueVisible: window.__seedGardenMoonFenceNextClueVisible ?? false,
    ...
    await page.screenshot({ path: `${OUT_DIR}/phaser-check-moon-fence-source-overview-393.png`, fullPage: false });
    await page.getByTestId("phaser-view-mode-toggle").click();
    await page.waitForTimeout(180);
    ...
    if (evidence.leaves !== "459") failures.push(`expected 459 leaves after moon grove harvest, got ${evidence.leaves}`);
    ...
    if (!expeditionSourcePreview.railText.includes("빈 밭에 초승달순 심기")) {
      failures.push("missing lunar source planting action after source preview");
    }
    if (!expeditionSourcePreview.receipts.some((receipt) => receipt.includes("달빛 울타리 route 잠김"))) {
      failures.push("missing moon fence route lock receipt");
    }
    ...
    if (moonFenceRouteUnlocked.moonFenceExpeditionState !== "ready") {
      failures.push(`expected moon fence expedition ready after route unlock, got ${moonFenceRouteUnlocked.moonFenceExpeditionState}`);
    }
    if (!moonFenceRouteUnlocked.railText.includes("월정 문 원정 보내기")) {
      failures.push("missing moon fence expedition start action after route unlock");
    }
    if (moonFenceExpeditionTraveling.activeExpeditionRouteId !== "expedition_moon_fence_unlocked") {
      failures.push(`expected moon fence active route after start, got ${moonFenceExpeditionTraveling.activeExpeditionRouteId}`);
    }
    if (moonFenceExpeditionTraveling.expeditionState !== "traveling") {
      failures.push(`expected generic expedition traveling during moon fence run, got ${moonFenceExpeditionTraveling.expeditionState}`);
    }
    if (moonFenceExpeditionTraveling.moonFenceExpeditionState !== "traveling") {
      failures.push(`expected moon fence expedition traveling, got ${moonFenceExpeditionTraveling.moonFenceExpeditionState}`);
    }
    if (moonFenceExpeditionTraveling.moonFenceRewardLeaves !== 88 || moonFenceExpeditionTraveling.expeditionRewardLeaves !== 88) {
      failures.push(`expected moon fence reward leaves 88 while traveling, got moon=${moonFenceExpeditionTraveling.moonFenceRewardLeaves} generic=${moonFenceExpeditionTraveling.expeditionRewardLeaves}`);
    }
    if (!moonFenceExpeditionTraveling.objective.includes("월정 문 원정 중")) {
      failures.push("missing moon fence traveling objective");
    }
    if (moonFenceExpeditionReturned.expeditionState !== "returned") {
      failures.push(`expected generic expedition returned for moon fence crate, got ${moonFenceExpeditionReturned.expeditionState}`);
    }
    if (moonFenceExpeditionReturned.moonFenceExpeditionState !== "returned") {
      failures.push(`expected moon fence expedition returned, got ${moonFenceExpeditionReturned.moonFenceExpeditionState}`);
    }
    if (!moonFenceExpeditionReturned.moonFenceReturnCrateVisible) {
      failures.push("missing moon fence return crate telemetry");
    }
    if (!moonFenceExpeditionReturned.railText.includes("월정 문 귀환 상자 열기")) {
      failures.push("missing moon fence return crate claim action");
    }
    if (moonFenceExpeditionClaimed.moonFenceExpeditionState !== "claimed") {
      failures.push(`expected moon fence expedition claimed, got ${moonFenceExpeditionClaimed.moonFenceExpeditionState}`);
    }
    if (!moonFenceExpeditionClaimed.moonFenceRewardClaimed) {
      failures.push("missing moon fence reward claimed telemetry");
    }
    if (!moonFenceExpeditionClaimed.moonFenceRewardMotionVisible) {
      failures.push("missing moon fence reward motion telemetry");
    }
    if (!moonFenceExpeditionClaimed.moonFenceNextClueVisible) {
      failures.push("missing moon fence next clue visibility");
    }
    if (moonFenceExpeditionClaimed.moonFenceNextClueId !== "clue_moon_grove_001") {
      failures.push(`expected moon fence next clue clue_moon_grove_001, got ${moonFenceExpeditionClaimed.moonFenceNextClueId}`);
    }
    ...
    if (moonFenceExpeditionClaimed.moonGroveSourceFxKey !== "fx_moon_grove_source_reward_strip_v1") {
      failures.push(`expected moon grove source FX key, got ${moonFenceExpeditionClaimed.moonGroveSourceFxKey}`);
    }
    ...
    if (moonGroveSourceAcquired.moonGroveSourceSeedPlanted) {
      failures.push("moon grove source planted before 월정 숲 심기 action");
    }
    if (moonGroveSourceAcquired.plotStates.some((plot) => plot.seedId === "seed_moon_grove_001")) {
      failures.push("seed_moon_grove_001 plot existed before 월정 숲 심기 action");
    }
    ...
    if (!moonGrovePlantAction.selectedText.includes("밭")) {
      failures.push(`expected an empty plot selected for moon grove planting, got ${moonGrovePlantAction.selectedText}`);
    }
    if (!moonGrovePlantAction.railText.includes("월정 숲 심기")) {
      failures.push("missing moon grove planting action after source acquisition");
    }
    if (!moonGrovePlantAction.moonGroveSourceSeedAvailable) {
      failures.push("moon grove source availability missing before planting action");
    }
    if (moonGrovePlantAction.moonGroveSourceSeedPlanted) {
      failures.push("moon grove source planted before planting click");
    }
    if (!moonGrovePlanted.moonGroveSourceAcquired) {
      failures.push("moon grove source acquisition disappeared after planting");
    }
    if (moonGrovePlanted.moonGroveSourceSeedAvailable) {
      failures.push("moon grove source seed availability was not consumed after planting");
    }
    if (!moonGrovePlanted.moonGroveSourceSeedPlanted) {
      failures.push("moon grove source planted telemetry missing after planting");
    }
    if (!moonGrovePlanted.moonGroveSourcePlotId) {
      failures.push("moon grove source plot id missing after planting");
    }
    const moonGrovePlot = moonGrovePlanted.plotStates.find((plot) => plot.seedId === "seed_moon_grove_001");
    if (!moonGrovePlot) {
      failures.push("missing seed_moon_grove_001 plot after moon grove planting");
    } else {
      if (moonGrovePlot.state !== "planted") {
        failures.push(`expected seed_moon_grove_001 planted state, got ${moonGrovePlot.state}`);
      }
      if (moonGrovePlot.growth !== 26) {
        failures.push(`expected seed_moon_grove_001 growth 26, got ${moonGrovePlot.growth}`);
      }
    }
    if (!moonGrovePlanted.objective.includes("seed_moon_grove_001") || !moonGrovePlanted.objective.includes("재배 중")) {
      failures.push("missing moon grove planted objective");
    }
    if (!moonGrovePlanted.receipts.some((receipt) => receipt.includes("seed_moon_grove_001 재배 시작"))) {
      failures.push("missing moon grove planted receipt");
    }
    const moonGroveReadyPlot = moonGroveReady.plotStates.find((plot) => plot.seedId === "seed_moon_grove_001");
    if (!moonGroveReadyPlot || moonGroveReadyPlot.state !== "ready" || moonGroveReadyPlot.growth !== 100) {
      failures.push(`expected seed_moon_grove_001 ready after care actions, got ${JSON.stringify(moonGroveReadyPlot)}`);
    }
    if (!moonGroveReady.railText.includes("월정 숲 수확")) {
      failures.push("missing moon grove harvest action");
    }
    if (!moonGroveReady.objective.includes("월정 숲 수확 준비")) {
      failures.push("missing moon grove ready objective");
    }
    if (moonGroveReady.moonGroveSourceSeedHarvested) {
      failures.push("moon grove source harvested before harvest click");
    }
    if (moonGroveReady.moonGroveDiscoveryRevealed) {
      failures.push("moon grove discovery revealed before harvest click");
    }
    if (!moonGroveReady.receipts.some((receipt) => receipt.includes("월정 숲 수확 준비 완료"))) {
      failures.push("missing moon grove ready receipt");
    }
    if (moonGroveHarvested.moonGroveSourceSeedPlanted) {
      failures.push("moon grove source planted telemetry stayed true after harvest");
    }
    if (!moonGroveHarvested.moonGroveSourceSeedHarvested) {
      failures.push("moon grove source harvest telemetry missing");
    }
    if (!moonGroveHarvested.moonGroveDiscoveryRevealed) {
      failures.push("moon grove discovery reveal telemetry missing");
    }
    if (moonGroveHarvested.moonGroveDiscoveryId !== "discovery_moon_grove_001") {
      failures.push(`expected moon grove discovery id discovery_moon_grove_001, got ${moonGroveHarvested.moonGroveDiscoveryId}`);
    }
    if (moonGroveHarvested.moonGroveDiscoveryName !== "월정 숲 새벽이끼") {
      failures.push(`expected moon grove discovery name 월정 숲 새벽이끼, got ${moonGroveHarvested.moonGroveDiscoveryName}`);
    }
    if (!moonGroveHarvested.moonGroveNextPreviewVisible) {
      failures.push("moon grove next preview telemetry missing");
    }
    if (!moonGroveHarvested.objective.includes("월정 숲 새벽이끼 발견")) {
      failures.push("missing moon grove harvest reveal objective");
    }
    if (!moonGroveHarvested.receipts.some((receipt) => receipt.includes("월정 숲 새벽이끼 발견"))) {
      failures.push("missing moon grove reveal receipt");
    }
    if (!moonGroveHarvested.receipts.some((receipt) => receipt.includes("discovery_moon_grove_001"))) {
      failures.push("missing moon grove discovery id receipt");
    }
    ...
    if (moonGroveHarvested.plotStates.some((plot) => plot.seedId === "seed_moon_grove_001")) {
      failures.push("seed_moon_grove_001 plot stayed occupied after moon grove harvest");
    }
    if (!moonGroveHarvested.receipts.some((receipt) => receipt.includes("discovery_moon_grove_001"))) {
    ...
    if (!moonGroveResearchHandoff.moonGroveClueMapAvailable) {
      failures.push("moon grove clue map action was not available after Miru handoff");
    }
    if (moonGroveResearchHandoff.moonGroveClueMapOpened) {
      failures.push("moon grove clue map opened before action");
    }
    if (!moonGroveResearchHandoff.railText.includes("숲길 지도 펼치기")) {
      failures.push("missing moon grove clue map action");
    }
    if (moonGroveClueMap.moonGroveClueMapAvailable) {
      failures.push("moon grove clue map availability stayed true after opening");
    }
    if (!moonGroveClueMap.moonGroveClueMapOpened) {
      failures.push("moon grove clue map opened telemetry missing");
    }
    if (moonGroveClueMap.moonGroveClueMapCurrentNodeId !== "research_moon_grove_path") {
      failures.push(`expected clue map current node research_moon_grove_path, got ${moonGroveClueMap.moonGroveClueMapCurrentNodeId}`);
    }
    if (moonGroveClueMap.moonGroveClueMapNextNodeId !== "route_moon_grove_greenhouse_path") {
      failures.push(`expected clue map next node route_moon_grove_greenhouse_path, got ${moonGroveClueMap.moonGroveClueMapNextNodeId}`);
    }
    if (moonGroveClueMap.moonGroveClueMapLockedNodeId !== "source_mist_greenhouse_silhouette") {
      failures.push(`expected clue map locked node source_mist_greenhouse_silhouette, got ${moonGroveClueMap.moonGroveClueMapLockedNodeId}`);
    }
    if (!moonGroveClueMap.objective.includes("온실 숲길 지도") || !moonGroveClueMap.objective.includes("물안개")) {
      failures.push("missing moon grove clue map objective");
    }
    if (!moonGroveClueMap.railText.includes("온실 숲길 지도") || !moonGroveClueMap.railText.includes("source_mist_greenhouse_silhouette")) {
      failures.push("missing moon grove clue map HUD surface");
    }
    if (moonGroveClueMap.lastFxKey !== "fx_research_clue_glimmer_strip_v1" || moonGroveClueMap.lastFxKind !== "moonGroveClueMap") {
      failures.push(`expected moon grove clue map FX binding, got ${moonGroveClueMap.lastFxKind}/${moonGroveClueMap.lastFxKey}`);
    }
    const miruClueMapActor = moonGroveClueMap.actorStates.find((actor) => actor.id === "actor_moon_grove_miru");
    if (
      !miruClueMapActor ||
      miruClueMapActor.role !== "researcher" ||
      miruClueMapActor.slotId !== "facility_research_shelf" ||
      miruClueMapActor.targetSlotId !== "facility_research_shelf" ||
      miruClueMapActor.task !== "research_clue"
    ) {
      failures.push(`expected Miru retained on clue map research shelf, got ${JSON.stringify(miruClueMapActor)}`);
    }
    const clueMapShelf = moonGroveClueMap.facilityStates.find((facility) => facility.slotId === "facility_research_shelf");
    if (!clueMapShelf || clueMapShelf.visualState !== "active" || clueMapShelf.progress < 88) {
      failures.push(`expected active research shelf after clue map, got ${JSON.stringify(clueMapShelf)}`);
    }
    if (!moonGroveClueMap.receipts.some((receipt) => receipt.includes("source_mist_greenhouse_silhouette"))) {
      failures.push("missing moon grove clue map receipt");
    }
    ...
    if (!moonFenceSourceOverview.hudCollapsed) {
      failures.push("moon fence source overview HUD did not collapse");
    }
    if (moonFenceSourceOverview.actionRailDisplay !== "none") {
      failures.push(`expected source overview action rail hidden, got ${moonFenceSourceOverview.actionRailDisplay}`);
    }
    if (moonFenceSourceOverview.bodyScrollHeight > moonFenceSourceOverview.innerHeight) {
      failures.push(
        `moon fence source overview body scrolled ${moonFenceSourceOverview.bodyScrollHeight}/${moonFenceSourceOverview.innerHeight}`
      );
    }
    if (!moonFenceSourceOverview.moonFenceNextClueVisible) {
      failures.push("moon fence source overview next clue telemetry missing");
    }
    ...
    if (moonFenceSourceOverview.moonGroveSourceSeedAvailable) {
      failures.push("moon fence source overview seed availability stayed true after planting");
    }
    ...
    if (moonFenceSourceOverview.moonGroveSourceSeedPlanted) {
      failures.push("moon fence source overview planted telemetry stayed true after harvest");
    }
    if (!moonFenceSourceOverview.moonGroveSourceSeedHarvested) {
      failures.push("moon fence source overview harvest telemetry missing");
    }
    if (!moonFenceSourceOverview.moonGroveDiscoveryRevealed) {
      failures.push("moon fence source overview discovery telemetry missing");
    }
    ...
    if (!moonFenceSourceOverview.moonGroveClueMapOpened) {
      failures.push("moon fence source overview clue map telemetry missing");
    }
    if (moonFenceSourceOverview.moonGroveClueMapCurrentNodeId !== "research_moon_grove_path") {
      failures.push(`expected overview clue map current node research_moon_grove_path, got ${moonFenceSourceOverview.moonGroveClueMapCurrentNodeId}`);
    }
    if (moonFenceSourceOverview.moonGroveClueMapNextNodeId !== "route_moon_grove_greenhouse_path") {
      failures.push(`expected overview clue map next node route_moon_grove_greenhouse_path, got ${moonFenceSourceOverview.moonGroveClueMapNextNodeId}`);
    }
    if (moonFenceSourceOverview.moonGroveClueMapLockedNodeId !== "source_mist_greenhouse_silhouette") {
      failures.push(`expected overview clue map locked node source_mist_greenhouse_silhouette, got ${moonFenceSourceOverview.moonGroveClueMapLockedNodeId}`);
    }
    ...
    if (moonFenceSourceOverview.plotStates.some((plot) => plot.seedId === "seed_moon_grove_001")) {
      failures.push("moon fence source overview seed_moon_grove_001 plot stayed occupied after harvest");
    }
    if (moonFenceSourceOverview.moonGroveSourceRenderedAssetKey !== "seed_moon_grove_001_icon") {
      failures.push(
        `expected source overview asset seed_moon_grove_001_icon, got ${moonFenceSourceOverview.moonGroveSourceRenderedAssetKey}`
      );
    }
    ...
    if (evidence.activeExpeditionRouteId !== "expedition_moon_fence_unlocked") {
    ...
    if (evidence.moonFenceExpeditionState !== "claimed") {
      failures.push(`expected final moon fence expedition claimed, got ${evidence.moonFenceExpeditionState}`);
    }
    if (evidence.moonFenceRewardLeaves !== 0) {
      failures.push(`expected final moon fence reward leaves 0, got ${evidence.moonFenceRewardLeaves}`);
    }
    if (evidence.moonFenceReturnCrateVisible) {
      failures.push("final moon fence return crate stayed visible after claim");
    }
    if (!evidence.moonFenceRewardClaimed) {
      failures.push("final moon fence reward claimed telemetry missing");
    }
    if (!evidence.moonFenceRewardMotionVisible) {
      failures.push("final moon fence reward motion telemetry missing");
    }
    if (!evidence.moonFenceNextClueVisible) {
      failures.push("final moon fence next clue telemetry missing");
    }
    ...
    if (evidence.moonGroveSourceSeedAvailable) {
      failures.push("final moon grove source seed availability stayed true after planting");
    }
    ...
    if (evidence.moonGroveSourceSeedPlanted) {
      failures.push("final moon grove source planted telemetry stayed true after harvest");
    }
    if (!evidence.moonGroveSourcePlotId) {
      failures.push("final moon grove source plot id missing");
    }
    if (!evidence.moonGroveSourceSeedHarvested) {
      failures.push("final moon grove source harvested telemetry missing");
    }
    if (!evidence.moonGroveDiscoveryRevealed) {
      failures.push("final moon grove discovery reveal telemetry missing");
    }
    if (evidence.moonGroveDiscoveryId !== "discovery_moon_grove_001") {
      failures.push(`expected final moon grove discovery id discovery_moon_grove_001, got ${evidence.moonGroveDiscoveryId}`);
    }
    if (evidence.moonGroveDiscoveryName !== "월정 숲 새벽이끼") {
      failures.push(`expected final moon grove discovery name 월정 숲 새벽이끼, got ${evidence.moonGroveDiscoveryName}`);
    }
    ...
    if (evidence.moonGroveClueMapAvailable) {
      failures.push("final moon grove clue map availability stayed true");
    }
    if (!evidence.moonGroveClueMapOpened) {
      failures.push("final moon grove clue map opened telemetry missing");
    }
    if (evidence.moonGroveClueMapCurrentNodeId !== "research_moon_grove_path") {
      failures.push(`expected final clue map current node research_moon_grove_path, got ${evidence.moonGroveClueMapCurrentNodeId}`);
    }
    if (evidence.moonGroveClueMapNextNodeId !== "route_moon_grove_greenhouse_path") {
      failures.push(`expected final clue map next node route_moon_grove_greenhouse_path, got ${evidence.moonGroveClueMapNextNodeId}`);
    }
    if (evidence.moonGroveClueMapLockedNodeId !== "source_mist_greenhouse_silhouette") {
      failures.push(`expected final clue map locked node source_mist_greenhouse_silhouette, got ${evidence.moonGroveClueMapLockedNodeId}`);
    }
    ...
    if (!evidence.topologyAssets.includes("fx_research_clue_glimmer_strip_v1")) {
      failures.push("missing research clue glimmer FX topology asset key");
    }
    ...
    if (evidence.lastFxKey !== "fx_research_clue_glimmer_strip_v1" || evidence.lastFxKind !== "moonGroveClueMap") {
      failures.push(`expected final moon grove clue map FX binding, got ${evidence.lastFxKind}/${evidence.lastFxKey}`);
    }
    if (evidence.leaves !== "459") {
      failures.push(`expected final leaves 459 after moon grove harvest, got ${evidence.leaves}`);
    }
    ...
    if (!evidence.objective.includes("온실 숲길 지도") || !evidence.objective.includes("물안개")) {
      failures.push("missing moon grove clue map final objective");
    }
    ...
    const moonGroveFinalPlot = evidence.plotStates.find((plot) => plot.seedId === "seed_moon_grove_001");
    if (moonGroveFinalPlot) {
      failures.push(`expected seed_moon_grove_001 plot to be cleared after harvest, got ${JSON.stringify(moonGroveFinalPlot)}`);
    }
    ...
    const result = {
      ...
      screenshots: [
        ...
        `${OUT_DIR}/phaser-check-moon-grove-plant-action-393.png`,
        ...
        `${OUT_DIR}/phaser-check-moon-grove-clue-map-opened-393.png`,
