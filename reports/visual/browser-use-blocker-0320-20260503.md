# Browser Use blocker — #320 새 기록 후속 수확 reveal payoff
...
Codex Browser Use 플러그인의 in-app browser(`iab`) 백엔드를 사용하기 위해 `browser-client.mjs`를 Node REPL에서 bootstrap하고 `agent.browser.nameSession("🔎 #320 새 기록 후속 수확 QA")`를 시도했다.
...
- 과거 blocker를 재사용하지 않고 #320 작업 중 현재 세션 재시도 결과를 별도 evidence로 남긴다.
...
Playwright screenshot은 `reports/visual/issue-320-album-record-followup-harvest-reveal-393.png`로 보존한다.
