...
첫 원정 route의 dedicated raster 후보 3개를 `assets/source/asset_plan.json`과 `assets/source/asset_prompts.json`에 추가한다.
...
다음 WorkUnit이 product decision 없이 원정 문/귀환 상자/귀환 보상 FX PNG 생성을 시작할 수 있다.
...
After: dedicated expedition asset 3개가 generation-ready plan/prompt로 추가된다. 이번 PR은 plan/prompt JSON 단계라 screenshot visual QA 대상이 아니다.
