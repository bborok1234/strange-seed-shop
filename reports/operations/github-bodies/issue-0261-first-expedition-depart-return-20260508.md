...
원정 문 preview 이후 첫 tutorial route `뒷마당 틈새길`을 출발/귀환 crate loop로 연결한다.

## Small win

플레이어가 원정 문을 단순 preview가 아니라 `보내기 -> 돌아옴 -> 상자 열기` long verb로 처음 경험한다.

## 사용자/운영자 가치

사용자는 research family reveal 이후 D7 원정 route의 실제 다음 행동을 이해한다. 운영자는 scripted smoke와 screenshot evidence로 원정 출발/귀환 상태를 재현할 수 있다.

## Before / After 또는 Visual evidence

Before: #488 이후 원정 문은 board/HUD preview로 보이지만 출발/귀환 route state가 없다.

After: `reports/visual/issue-0490-first-expedition-depart-return/`에 원정 출발, 귀환 상자, 보상 수령 screenshot을 저장한다.

Browser Use: 현재 세션에서 `iab` 실행 도구가 노출되지 않아 blocker를 보고서에 기록하고 Playwright fallback을 사용한다.
