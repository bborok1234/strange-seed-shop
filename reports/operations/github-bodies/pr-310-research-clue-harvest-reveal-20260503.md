...
#310 연구 단서 source plot 수확 순간을 일반 수확이 아니라 `단서 생명체 발견` reveal로 보이게 만들었습니다. 수확 overlay에 `도감 단서 기록` receipt가 붙고, overlay를 닫은 뒤 정원 생산 장면에 `단서 생명체 발견`/`도감 단서 기록` playfield state가 남습니다.

## Small win

- 이번 PR이 만든 가장 작은 승리: 연구 단서 씨앗을 수확하면 “다음 생명체를 찾아냈다”는 보상 장면과 다음 목표 전환을 즉시 봅니다.

## Plan-first evidence

- Plan artifact: `items/0157-research-clue-harvest-reveal.md`
- Plan에서 벗어난 변경이 있다면 이유: 없음. 신규 manifest asset 없이 DOM/CSS reward motion과 visual regression만 추가했습니다.

## Game Studio route

- Umbrella: `game-studio:game-studio`
- Specialist route: `game-studio:game-ui-frontend` + `game-studio:game-playtest`
- 적용한 playfield/HUD/playtest 기준: 수확 reveal이 viewport 안에 들어오고, overlay 종료 후 playfield reward state가 action surface를 가리지 않으며, 393px bottom-tab overlap/action-surface overflow invariant를 확인했습니다.
...
- 게임 가치: 연구 단서 심기 → 성장 예고 → 수확 발견까지 first-5-minute collection arc가 끊기지 않습니다.
- 운영사 가치: queue-empty 이후에도 GitHub-authoritative WorkUnit/PR/CI evidence로 production game quality chain을 전진시킵니다.

## Before / After 또는 Visual evidence

- Before: 연구 단서 씨앗을 수확해도 일반 수확 reveal과 비슷해 “단서로 찾은 생명체” 감정이 약했습니다.
- After: overlay aria-label `단서 생명체 발견`, `도감 단서 기록` receipt, `연구 단서 수확` card, 다음 목표 전환, playfield `단서 생명체 발견` state가 보입니다.
- Browser Use evidence 또는 blocker: `reports/visual/browser-use-blocker-0310-20260503.md` — 현재 Codex 세션 iab backend discovery 실패.
...
- 더 강한 sprite/FX 수확 burst가 필요하면 별도 asset provenance WorkUnit으로 분리해야 합니다.
