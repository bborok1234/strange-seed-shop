...
test("모바일 자동 생산과 첫 주문은 수령과 납품 CTA를 한 화면에서 검증한다", async ({ page }, testInfo) => {
  ...
  await expect(page.locator(".production-fx-production")).toBeVisible();
  ...
  await expect(page.locator(".production-fx-order")).toBeVisible();
  await page.screenshot({ path: testInfo.outputPath("mobile-production-order-fx-v0-393.png"), fullPage: false });
