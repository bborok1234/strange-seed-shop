...
interface GreenhouseIrrigationEntryReceipt {
  id: number;
  nextOrderTitle: string;
  bonusPercent: number;
}
...
const MERCHANT_FOLLOWUP_ORDER: FirstOrderDefinition = {
  id: "order_merchant_leaf_bundle_001",
  title: "포장잎 상인 단골 납품",
  customer: "포장잎 상인의 새 주문상자",
  requiredLeaves: 18,
  rewardLeaves: 54,
  rewardPollen: 2,
  rewardMaterials: 1
...
export default function App() {
  ...
  function showAlbumRecordPlantReceipt(seed: SeedDefinition, actionLabel: string) {
    if (!researchAlbumRecord || !nextCreatureGoal || seed.id !== nextCreatureGoal.seed.id) {
      return;
    }

    const receipt: AlbumRecordPlantReceipt = {
      id: Date.now(),
      seedId: seed.id,
      seedName: seed.name,
      creatureName: nextCreatureGoal.creature.name,
    ...
    setResearchHarvestReceipt(null);
    setAlbumRecordPlantReceipt(receipt);
    window.setTimeout(() => {
      setAlbumRecordPlantReceipt((current) => (current?.id === receipt.id ? null : current));
    }, 6_000);
  }
  ...
  function buySeed(seed: SeedDefinition) {
    ...
    const isAlbumRecordTargetPurchase = Boolean(researchAlbumRecord && nextCreatureGoal?.seed.id === seed.id && activeTab === "seeds");
    if (canPurchase && !isAlbumRecordTargetPurchase) {
  ...
  function plantOwnedSeed(seed: SeedDefinition) {
    ...
    const shouldOpenGardenAfterPlanting =
      ...
      (getSeedPlantingSource(save, seed) === "greenhouse_mist" || isResearchClueSeed || isAlbumRecordTargetSeed);
    const canPlant =
      save !== null &&
      (save.seedInventory[seed.id] ?? 0) > 0 &&
      save.plots.some((candidate) => candidate.index < save.plotCount && !candidate.seedId);
    ...
    if (canPlant && isAlbumRecordTargetSeed) {
  ...
  function claimMerchantCrateReward() {
    ...
    commit((draft) => {
      ...
      draft.idleProduction.pendingLeaves = Math.max(
        ...
        MERCHANT_FOLLOWUP_ORDER.requiredLeaves
      );
      draft.idleProduction.lastTickAt = new Date(now).toISOString();
  ...
  function buyGreenhouseIrrigation() {
    const canBuild =
      ...
      save.greenhouseIrrigationLevel < GREENHOUSE_IRRIGATION_MAX_LEVEL &&
      save.idleProduction.completedOrderIds.includes(GREENHOUSE_ROUTE_SUPPLY_ORDER.id) &&
      save.materials >= GREENHOUSE_IRRIGATION_COST_MATERIALS &&
      save.pollen >= GREENHOUSE_IRRIGATION_COST_POLLEN;
    commit((draft) => {
      if (
    ...
    triggerRewardPulse();
    if (canBuild) {
      const entryReceipt: GreenhouseIrrigationEntryReceipt = {
        id: Date.now(),
        nextOrderTitle: GREENHOUSE_IRRIGATION_ORDER.title,
        bonusPercent: Math.round(GREENHOUSE_IRRIGATION_RATE_BONUS * 100)
      };
      setGreenhouseIrrigationEntryReceipt(entryReceipt);
      window.setTimeout(() => {
        setGreenhouseIrrigationEntryReceipt((current) =>
          current?.id === entryReceipt.id ? null : current
        );
      }, 2_000);
      trackEvent("greenhouse_irrigation_entry_revealed", {
        rewardMotion: "greenhouse_irrigation_entry_reveal",
        nextOrderId: GREENHOUSE_IRRIGATION_ORDER.id
      });
    }
  }
          ...
          <aside className={actionSurfaceClassName}>
            ...
            {productionStatus && productionStatus.ratePerMinute > 0 && (
              ...
              >
                ...
                {albumRecordPlantReceipt && (
                  <div className="album-record-plant-receipt" aria-label="새 기록 후속 재배">
                    <span className="album-record-plant-chip">새 기록 후속 재배</span>
                ...
                {greenhouseIrrigationEntryReceipt && (
                  <div
                    className="greenhouse-irrigation-entry-receipt"
                    aria-label="물길 점검 완료 보상"
                  >
                    <span className="greenhouse-irrigation-entry-chip">물길 점검 완료</span>
...
function buildGardenPlayfieldViewModel(
  ...
  const plots = save.plots.map((plot): GardenPlotView => {
    ...
    return {
      ...
      family: seed.family,
      source: plot.source,
      sourceLabel: albumRecordSeedSource ? "새 기록 후속 재배" : greenhouseLunarSource ? "온실 단서" : researchSeedSource ? "연구 단서" : undefined,
  ...
  return {
    ...
    hint:
      albumRecordPlantActive
        ? `${albumRecordPlantSeedName}을 톡톡 키워 다음 도감 기록으로 이어가세요`
        : readyCount > 0
...
function getCurrentOrder(save: PlayerSave): FirstOrderDefinition {
  ...
  if (
    save.claimedMerchantCrateRewardIds.includes(getMerchantCrateRewardId(MERCHANT_RECORD_CREATURE_ID)) &&
    !save.idleProduction.completedOrderIds.includes(MERCHANT_FOLLOWUP_ORDER.id)
  ) {
    return MERCHANT_FOLLOWUP_ORDER;
  }
