...
Game Studio 기준에서 다음 단계는 `Phaser playfield + React DOM HUD` 구조를 검증하는 작은 spike다. 저장, 경제, 콘텐츠, analytics 계약은 유지하고 중앙 정원 playfield만 2D 게임 런타임으로 분리한다.
...
- 공통 action boundary: `plant_seed`, `tap_growth`, `harvest_plot`, `claim_album_reward`.
...
- 첫 세션 루프는 유지된다: starter seed -> plant -> tap growth -> harvest -> album reward -> unlock second plot.
- 360x800 garden viewport에서 core loop는 세로 스크롤 없이 조작 가능하다.
- Browser Use/CDP evidence가 `reports/visual/`에 저장된다.

## 다음 작업

`items/0016-phaser-playfield-runtime-spike.md`를 기준으로 아주 작은 runtime spike를 만든다. 이 작업 전에는 `items/0015-design-system-foundation.md`의 CSS polish를 완성하려고 더 밀지 않는다. 0015는 guardrail과 임시 HUD rescue로 남기고, 0016이 게임 느낌을 결정하는 구조 검증 역할을 맡는다.
