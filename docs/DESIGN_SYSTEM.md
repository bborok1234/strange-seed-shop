...
- 모바일 non-garden 탭은 viewport 상단부터 bottom nav 위까지 완전히 덮는 독립 game screen이다. 정원 HUD/playfield는 뒤에 비쳐 보이거나 터치되면 실패다.
- 하단 탭은 Material bottom navigation 원칙처럼 3~5개의 최상위 destination으로만 사용하고, 각 destination은 modal/dialog가 아니라 고정 view로 전환한다.
- 주요 탭/CTA touch target은 Android Accessibility 권장치인 48dp에 가깝게 유지한다.
...
- 모바일은 하나의 세로 game frame이다. 정원과 도감/씨앗/원정/상점은 동시에 보이는 대시보드가 아니라, 하단 탭으로 전환되는 독립 화면이다.
