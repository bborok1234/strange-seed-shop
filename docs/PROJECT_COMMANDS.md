...
Studio run is infinite by default. Studio Harness v3 foreground operator(`npm run studio:v3:operate`), Studio Harness v3 runner, 또는 명시적 v3 운영 루프가 시작되면 issue 생성, PR 생성, merge, Release, Retro, daily report는 모두 루프 내부 checkpoint다. 이것들은 완료나 종료 사유가 아니다.
...
Studio Harness v3 foreground operator는 `$ralph`의 상태/검증 계약을 참고하지만, Codex App에서 `$ralph`를 입력했다는 사실만으로 장시간 runner가 실행 중이라고 보지 않는다.
...
Studio Harness v3 foreground operator에서 assistant `final` 응답은 세션을 닫는 terminal action이다. `final response is terminal`을 운영 계약으로 보고, stop rule 없이 `final` 응답을 보내지 않는다.
...
이 프로젝트는 일반 웹앱이 아니라 브라우저 게임이다. Studio Harness v3 foreground operator가 게임 기능, UI/HUD, playfield, asset, QA, playtest issue를 처리할 때는 Game Studio plugin route가 필수다.
...
Studio Harness v3 foreground operator는 직전 issue에서 이어지는 작은 기능을 자동 선택하지 않는다. 새 게임 issue plan은 최소 3개 후보를 비교하고, 그중 하나는 큰 방향 점프 후보여야 한다.
...
Studio Harness v3 foreground operator는 역할별 독립 산출물이 있을 때 Codex native subagents 또는 team mode를 사용한다.
...
Studio Harness v3 foreground operator는 작은 기능 개선을 기본값으로 고르지 않는다. 새 게임 issue 후보는 아래 5개 중 최소 3개를 plan에 명시해야 한다.
...
GitHub issue/PR/comment 본문은 코드와 같은 운영사 산출물이다. Studio Harness v3 foreground operator에서 GitHub metadata를 만들거나 고칠 때는 아래를 지킨다.
...
GitHub required checks는 빠르고 재현성 높은 `npm run check:ci`를 실행한다. Browser Use와 Playwright screenshot 기반 visual QA는 Studio Harness v3 foreground operator 작업 과정의 evidence gate로 남기며, UI/visual 변경 PR 본문과 `reports/visual/`에 결과 또는 blocker를 기록한다.
...
화면/UI/visual quality 비판이 포함되면 설계 판단 전에 Browser Use preflight를 먼저 수행한다. `browser-use` namespace가 보이지 않아도 실패가 아니며, `tool_search`로 Node REPL `js`를 노출하고 `iab` backend bootstrap을 시도한다. Computer Use가 먼저 보인다는 이유로 Browser Use를 건너뛰지 않는다.
...
7. 구현이 명확해지면 Studio Harness v3 foreground operator용 issue와 plan-first artifact로 넘긴다.
...
- 사용자가 “계속 달려”, “운영모드”, “북극성까지”라고 하면 Studio Harness v3 foreground operator(`npm run studio:v3:operate`)로 본다.
...
명령어가 섞이면 가장 안전한 모드가 우선이다. 예를 들어 `$seed-brief`와 v3 운영 요청이 함께 있으면 먼저 brief로 현재 상태를 고정한 뒤, stop rule이 없을 때 `npm run studio:v3:operate` 기준의 v3 operator로 이어간다.
