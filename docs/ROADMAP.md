...
| Seed ops final publication ask regression | active | `items/0134-seed-ops-final-publication-ask-regression.md`, `.codex/skills/seed-ops/SKILL.md`, `scripts/check-seed-ops-queue-gate.mjs`, `scripts/operator-control-room.mjs` | GitHub issue/PR 게시 경계에서 `final`로 확인을 묻는 패턴을 하네스 회귀로 고정하고, commentary checkpoint + heartbeat + same-turn local continuation action을 요구한다 |
