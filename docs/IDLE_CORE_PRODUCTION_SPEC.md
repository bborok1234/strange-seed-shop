...
### Economy Affordance Contract

경제 행동은 숫자가 아니라 의사결정이다. 플레이어가 버튼을 누르기 전에 비용, 보유량, 결과, 부족 사유를 한눈에 판단하지 못하면 core loop가 끊긴다.

기획팀 acceptance:

- 모든 `구매`, `강화`, `연구`, `원정`, `납품`, `심기` CTA는 재화 단위를 명시한다. 예: `25 잎`, `꽃가루 3`, `재료 1`.
- 같은 화면 안에서 현재 보유량을 함께 보여준다. 예: `보유 72 잎`, `재료 1/2`.
- 버튼 또는 row는 구매 후 결과를 말한다. 예: `구매하고 심기`, `보관 12 -> 24`, `연구 시작`.
- 부족 상태는 disabled + 부족 수량으로 표시한다. 예: `13 잎 부족`.
- `구매 25`, `강화 2`, `필요 1`처럼 단위 없는 숫자만 남긴 UI는 production bar를 통과하지 못한다.
