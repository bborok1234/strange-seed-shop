...
`Agent Automerge Trial` workflow는 PR이 자동 머지 후보인지 판정하고, 저장소 변수가 켜진 경우에만 GitHub native auto-merge를 요청한다. Draft PR 또는 `agent-automerge` label이 없는 PR은 자동 머지 후보가 아니므로 후보 판정 실패로 PR을 빨갛게 만들지 않고, `Automerge is intentionally skipped`를 기록한다. CI용 기본 검증인 `npm run check:ci`는 `.github/workflows/ci.yml`의 `Verify game baseline`이 단독으로 소유한다. 실제 GitHub native auto-merge 요청은 저장소 변수 `ENABLE_AGENT_AUTOMERGE`가 `true`일 때만 실행된다.
