...
모든 CTA와 구매/강화 row는 누르기 전부터 아래 4가지를 설명해야 한다.

- Cost: 어떤 재화를 얼마나 쓰는지. 예: `25 잎`, `재료 2`.
- Balance: 현재 그 재화를 얼마나 갖고 있는지. 예: `보유 72 잎`.
- Result: 누르면 무엇이 생기는지. 예: `구매 후 정원에 심기`, `보관 12 -> 24`.
- Blocker: 불가능하면 왜 불가능한지. 예: `13 잎 부족`, `밭을 비워야 해요`.

`구매 25`처럼 숫자만 있는 CTA는 실패다. 숫자만 보여주면 플레이어는 이 값이 잎인지, 꽃가루인지, 재료인지, 현재 감당 가능한지, 구매 후 무엇이 바뀌는지 알 수 없다. 기획팀은 새 화면을 승인하기 전에 모든 경제 행동이 이 4요소를 만족하는지 확인한다.
...
- 구매 가능/보유/부족 사유
- 가격 재화 단위와 현재 보유량. 예: `25 잎 · 보유 72 잎`
- 예상 성장 시간
- 연결된 생명체 단서
- 정원으로 돌아가는 심기 행동
- 구매 후 결과. 예: `구매하고 심기`, `보유 씨앗 정원에 심기`
