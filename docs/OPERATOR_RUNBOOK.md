...
- PR publication boundary: Routine GitHub issue/PR/comment publication은 Studio Harness v3 runner responsibility다. credentials/tools가 가능하면 `gh issue create/edit/comment`, `gh pr create/edit/comment/ready/merge`, branch push, checks 확인, merge를 agent가 실행한다. 단순 GitHub 게시에 action-time confirmation wait를 self-impose하거나 final로 게시 확인을 물으면 harness regression이다. PublicationBoundary는 credential/tool/runtime blocker 또는 destructive/external-production/payment/customer-data boundary가 실제로 막을 때만 기록한다. 실제 blocker가 있으면 heartbeat/control room/report/checker/next plan으로 continuation을 남기고, routine publication은 `confirmation.channel: preapproved`와 다음 agent action으로 표현한다.
...
Codex App의 PR publication boundary는 단독 stop 사유가 아니다. 이 레포의 routine GitHub issue/PR/comment publication은 preauthorized runner responsibility이며, action-time confirmation wait를 self-impose하지 않는다. assistant final publication ask is a regression: final로 GitHub 게시 확인을 묻지 않는다. 실제 credential/tool/runtime blocker가 있을 때만 pending external-publication gate를 보고서로 고정하고 로컬 연속 작업을 먼저 찾는다.
